...
public class StartingListController : ControllerBase
{
    ...
    public async Task<IActionResult> ConfirmStartingList(int raceId)
    {
        ...
        try
        {
            var result = await _startingListService
                .ConfirmStartingListAsync(refereeId, raceId);
            ...
            when (ex.Message == "RACE_NOT_UPCOMING")
        {
            return Conflict(new
            {
                ...
                message = "Starting list can only be confirmed before the race starts."
            });
        }
        catch (InvalidOperationException ex)
            when (ex.Message == "NO_RACE_ENTRIES")
        {
            return Conflict(new
            {
                error = "NO_RACE_ENTRIES",
                message = "Race has no race entries."
            });
        }
        catch (InvalidOperationException ex)
            when (ex.Message == "NO_ELIGIBLE_STARTING_ENTRIES")
        {
            return Conflict(new
            {
                error = "NO_ELIGIBLE_STARTING_ENTRIES",
                message = "There are no eligible race entries for the official starting list."
            });
        }
    }
}