...
**Mã dự án:** SU26SWP03
**Phiên bản:** 1.5 (Consolidated)
**Ngày:** 11/06/2026
**Chuẩn áp dụng:** IEEE 29148:2018 / IEEE 830-1998 / UML 2.5
**Trạng thái:** Đã qua Conflict Audit & Fix (xem CONFLICT_AUDIT_REPORT.md)

---

> **Ghi chú tích hợp:** Tài liệu này được tổng hợp từ ba file nguồn đã được sửa xung đột:
> - `INTRODUCTION v1.3` (sửa XĐ-01, XĐ-02, XĐ-06, XĐ-07)
> - `OVERALL DESCRIPTION v1.5` (sửa XĐ-02, XĐ-04, XĐ-05, XĐ-08)
> - `SPECIFIC REQUIREMENT v1.5` (sửa XĐ-03)
...
- 3.2 Functional Requirements — 109 FR theo 17 Module A–Q (REQ-F-ACC → REQ-F-SEC)
...
- **Onboarding Referee & Doctor (EC-37):** Hồ sơ Race Referee và Doctor khởi tạo ở trạng thái `Pending`; Admin phải duyệt onboarding (đối chiếu chứng chỉ trọng tài / giấy phép hành nghề thú y) để chuyển sang `Active`. **Chỉ tài khoản `Active`** mới được phân công nhiệm vụ (Referee — Module F; Doctor — Module G), tránh tình trạng hồ sơ kẹt `Pending` gây bế tắc Pha 3/4 hoặc dùng người chưa thẩm định chứng chỉ.
...
- **Vòng đời trạng thái giải đấu (EC-36):** `Tournaments.Status` chuyển tiếp theo state machine có tác nhân/điều kiện rõ ràng: `Draft → Open Registration` (Admin công bố) → `Closed Registration` (đóng đăng ký/tới hạn) → `Pre-Race` (hoàn tất lập lịch + phân công) → `In-Progress` (Race đầu tiên `Live`) → `Completed` (mọi Race `Official`/`Cancelled`). Mỗi chuyển tiếp được ghi Audit Log; trạng thái cấp giải tách bạch với trạng thái cấp cuộc đua.
...
- **Thời điểm kiểm tra kinh nghiệm (EC-21):** Vì một cặp Owner–Jockey (Pairing) là độc lập với giải đấu, ràng buộc `Jockey.ExperienceYears ≥ Tournament.MinJockeyExperienceYears` phải được **tái kiểm tra tại thời điểm đưa cặp vào cuộc đua cụ thể** (`RaceEntry`, Pha Lập lịch) — nơi đã xác định được giải đấu — không chỉ kiểm tra một lần ở bước gửi lời mời.
...
- **Các loại giao dịch VirtualPointsTransactions (`Type`):** `SignUp Bonus` (1000 điểm khởi tạo), `Prediction Placed` (âm — trừ khi đặt cược), `Prediction Win Reward` (+200 điểm), `Prediction Refund` (hoàn trả khi cặp bị Disqualified hoặc Withdrawn).
...
| BR-05 | Kinh nghiệm tối thiểu Jockey | D | `Jockey.ExperienceYears ≥ Tournament.MinJockeyExperienceYears` — hệ thống chặn mời nếu không đủ; **(EC-21)** tái kiểm tra tại thời điểm tạo `RaceEntry` (Pha 3) vì Pairing độc lập với giải đấu |
...
| BR-45 | State machine cấp giải (EC-36) | B | `Tournaments.Status` 5 chuyển tiếp có tác nhân/điều kiện (Draft→Open→Closed→Pre-Race→In-Progress→Completed), mỗi chuyển tiếp ghi AuditLog; tách bạch trạng thái cấp Race |
| BR-46 | Onboarding Referee & Doctor (EC-37) | A, F, G | Hồ sơ khởi tạo `Pending`; Admin duyệt (đối chiếu chứng chỉ) → `Active`; chỉ `Active` mới được phân công (Referee Node 3.5, Doctor `DoctorAssignments`) |
...
HRTMS vận hành theo đúng sáu pha nghiệp vụ tuần tự, bám sát 17 module A-Q trong Scope.

| **Pha** | **Tên pha** | **Module chính** | **Actors chính** | **Kết quả chuyển pha** |
| --- | --- | --- | --- | --- |
| **Pha 1** | Khởi tạo giải đấu | A, B, K, O | Admin, tất cả Actors | Giải đấu mở đăng ký |
| **Pha 2** | Đăng ký tham gia | C, D, O | Horse Owner, Jockey, Admin | Ngựa và Jockey hợp lệ được ghép cặp |
| **Pha 3** | Lập lịch & chuẩn bị | E, F, L, M, O | Admin, Horse Owner, Jockey, Race Referee, Spectator | Lịch, vị trí xuất phát, trọng tài và cổng dự đoán sẵn sàng |
| **Pha 4** | Kiểm tra trước thi đấu | G, M, O | Admin, Doctor, Race Referee | Danh sách xuất phát chính thức được xác nhận hoặc loại khẩn cấp |
| **Pha 5** | Theo dõi diễn biến cuộc đua | H, I, O | Race Referee, Spectator, Horse Owner, Jockey | Kết quả sơ bộ và biên bản thi đấu được lập |
| **Pha 6** | Công bố kết quả & tính thưởng | J, K, L, N, P, Q, O | Admin, Spectator, tất cả Actors theo quyền | Kết quả Official, điểm ảo, Leaderboard, báo cáo và Audit Log được cập nhật |

**Trạng thái chuyển tiếp (EC-10 SYNC — tách 2 state machine, giá trị khớp CHECK constraint trong Database Schema):**

- **Cấp Giải đấu (`Tournaments.Status`):** `[Draft] → [Open Registration] → [Closed Registration] → [Pre-Race] → [In-Progress] → [Completed]`
- **Cấp Cuộc đua (`Races.Status`):** `[Upcoming] → [Live] → [Unofficial] → [Official]` *(+ `Cancelled` khi toàn bộ cặp đấu rút lui trước giờ chạy)*

> **Tác nhân/điều kiện chuyển trạng thái giải (EC-36 — BR-45):** `Draft → Open Registration` (Admin công bố, Node 1.5); `→ Closed Registration` (Admin đóng đăng ký / tới hạn); `→ Pre-Race` (hoàn tất lập lịch + phân công Referee/Doctor Pha 3); `→ In-Progress` (Race đầu tiên `Live`); `→ Completed` (mọi Race `Official`/`Cancelled`). Mỗi chuyển tiếp ghi AuditLog; tránh giải kẹt vĩnh viễn ở `Open Registration`.

---

### **2.2.2 Pha 1: Khởi Tạo Giải Đấu (Tournament Initialization)**

**Mục đích:** Admin thiết lập giải đấu, cấu trúc vòng đua/cuộc đua, quỹ tiền thưởng và công bố giải.

`[Node 1.1] Đăng ký tài khoản & phân quyền theo Role ↓ (Actor: Tất cả Actors | Module A) [Node 1.2] Admin tạo giải đấu và thông số đặc thù ↓ (Module B: AllowedBreed — chọn đúng 1 giống ngựa duy nhất (single-select), TrackType, RaceDistance, RaceCategory, MinJockeyExperienceYears) [Node 1.3] Admin lập cấu trúc Round/Race ↓ (Module B) [Node 1.4] Admin cấu hình Purse ↓ (Module K) [Node 1.5] Công bố giải đấu và gửi thông báo ↓ (Module B + O) Trạng thái: Open Registration`

> **Luồng hủy giải (EC-30 — BR-27):** Admin có thể hủy giải trước khi bắt đầu chính thức; nếu giải đã có dữ liệu phát sinh, hệ thống thực hiện 1 giao dịch nhất quán: cancel mọi RaceEntry, refund mọi Prediction `Pending`, vô hiệu Pairings/PursePayouts, gửi Notification và ghi AuditLog.

---

### **2.2.3 Pha 2: Đăng Ký Tham Gia (Registration Phase)**

**Mục đích:** Jockey khai báo chứng chỉ, Horse Owner khai báo hồ sơ ngựa, Admin phê duyệt hồ sơ và Owner ghép cặp Jockey.

`[Jockey tự khai báo: chứng chỉ hành nghề, ExperienceYears, cân nặng sơ bộ, tình trạng sức khỏe, và quan hệ gia đình ruột thịt liên quan ngành đua ngựa — bắt buộc, phục vụ Independence Check Pha 4] ↓ Admin phê duyệt hồ sơ Jockey trước khi Jockey xuất hiện trong danh sách tìm kiếm [Horse Owner tự khai báo: thông tin hành chính ngựa (tên, BirthYear, giới tính, màu sắc, huyết thống, trọng lượng, đặc điểm nhận dạng) và thông tin y tế (Breed, VaccinationRecordRef, DopingTestDate, DopingTestResult: Clean/Pending/Failed) — Doctor không tham gia bước này] ↓ Admin kiểm duyệt hồ sơ ngựa [Hệ thống tự động đối chiếu Horse.Breed với AllowedBreed duy nhất của giải (single-value); nếu không khớp → Auto-reject tức thì. DopingTestResult = Failed → Auto-reject tức thì.] ↓ Nếu hợp lệ [Hệ thống tạo RaceEntry với EntryFeeStatus = 'Unpaid' (hoặc tự động 'Paid' nếu Tournament.EntryFeeAmount = 0)] ↓ [Horse Owner thanh toán Entry Fee ngoài hệ thống → Admin xác nhận và cập nhật EntryFeeStatus = 'Paid'] ↓ Admin Approve hồ sơ (Node 2.5 — bị block nếu EntryFeeStatus != 'Paid') [Horse Owner gửi lời mời Jockey] ↓ Jockey đồng ý hoặc từ chối [Cặp Ngựa–Jockey được xác nhận]`

| **Unhappy case** | **Xử lý** |
| --- | --- |
| Jockey bị từ chối | Jockey không xuất hiện trong danh sách tìm kiếm của Horse Owner |
| Giống ngựa không khớp AllowedBreed | Hệ thống Auto-reject ngay khi Horse.Breed không khớp với AllowedBreed duy nhất đã cấu hình của giải (single-value). Admin không thể override kết quả Auto-reject này. |
| DopingTestResult = Failed được Owner khai báo | Hệ thống tự động Auto-reject, không cần Admin xét duyệt |
| Admin từ chối hồ sơ ngựa | Bắt buộc nhập lý do từ chối tối thiểu 10 ký tự và gửi thông báo cho Horse Owner |
| Jockey từ chối lời mời | Horse Owner tìm và gửi lời mời Jockey khác |

---

### **2.2.4 Pha 3: Lập Lịch & Chuẩn Bị (Scheduling & Pre-Race Preparation)**

**Mục đích:** Admin phân bổ ngựa vào cuộc đua, bốc thăm vị trí xuất phát, Owner xác nhận tham gia, Admin phân công trọng tài và mở cổng dự đoán đúng điều kiện.

`[Admin phân bổ RaceEntry vào Race] ↓ [Post Position Draw ngẫu nhiên và công khai] ↓ [Horse Owner xác nhận tham gia trước Confirmation Cut-off Time] ↓ [Admin phân công Lead Referee + Assistant Referees] ↓ Referee COI Check [Referee nhận phân công] ↓ [Prediction Gate chỉ mở sau Post Position Draw] ↓ [Spectator đặt dự đoán Win]`

| **Ràng buộc** | **Nội dung** |
| --- | --- |
| Withdrawal Flow | Nếu quá hạn xác nhận hoặc Owner rút lui, RaceEntry chuyển Cancelled, PostPosition chuyển Vacant, **Spectator có dự đoán vào cặp đó được hoàn điểm ảo tự động (Prediction Refund)**, Admin nhận thông báo khẩn và sự kiện được ghi Audit Log |
| Referee COI Check | Trọng tài không được là người thân trực hệ của bất kỳ Horse Owner nào có ngựa trong cuộc đua |
| Prediction Gate | Chỉ mở khi isPostPositionDrawn = true |
| Prediction Type | Chỉ hỗ trợ Win (Top 1) |
| Form Score | Static SQL: 40% lịch sử ngựa, 35% lịch sử Jockey, 25% kết quả trung bình theo loại vòng đua |
| Tái kiểm tra kinh nghiệm (EC-21) | BR-05 được tái kiểm tra tại thời điểm đưa cặp vào Race cụ thể (`RaceEntry`) — nơi đã xác định giải đấu, không chỉ ở bước gửi lời mời |
| Re-run COI (EC-25) | Khai báo quan hệ gia đình của Referee thay đổi sau phân công → tự động chạy lại COI cho mọi phân công active; xung đột → đình chỉ + Alert Admin |
| Số ngựa tối thiểu (EC-16) | Không áp ngưỡng — "ngựa bao nhiêu đua bấy nhiêu"; purse khi thiếu vị trí xử lý theo BR-26 (Remainder) |
| Số ngựa tối đa (EC-46 — BR-55) | `Tournament.MaxHorses` per-Race; chặn duyệt/tạo RaceEntry khi đạt giới hạn |
| Chống trùng trong cùng Race (EC-40 — BR-49) | Chặn một `HorseId`/`JockeyId` (qua Pairing) vào cùng một Race hai lần; khác EC-15 (Race chồng lấn thời gian) |
| Phân công & COI Doctor (EC-38 — BR-47) | Doctor phải được Admin phân công cho Race (`DoctorAssignments`) và đạt Doctor COI Check trước khi cân/khám; chỉ Doctor `Active` (EC-37) |
| Một Lead Referee/Race (EC-45 — BR-54) | Mỗi Race đúng 1 Lead Referee (filtered UNIQUE) |
| Ràng buộc thời gian lập lịch (EC-35 — BR-44) | `StartDate ≤ Round.ScheduledDate ≤ Race.ScheduledTime ≤ EndDate`, `> Now` |

---

### **2.2.5 Pha 4: Kiểm Tra Trước Thi Đấu (Pre-Race Medical & COI Check)**

**Mục đích:** Doctor và Race Referee thực hiện kiểm tra trước giờ đua; hệ thống loại khẩn cấp cặp thi đấu không đạt điều kiện.

`[Doctor cân Jockey trước đua — Pre-Race Weigh-In (Node 4.1-MD)] ↓ Ghi PreRaceJockeyWeight; điều kiện bắt buộc trước khi chuyển sang Live [Doctor xác minh thủ công danh tính ngựa tại Paddock (Node 4.2)] ↓ Kết quả: Matched ↓ tiếp tục | Mismatch ↓ Disqualified khẩn cấp [Doctor khám lâm sàng và ghi kết luận Fit/Unfit (Node 4.3)] ↓ Kết quả: Fit ↓ tiếp tục | Unfit (kèm lý do ≥ 20 ký tự) ↓ Disqualified khẩn cấp [Race Referee kích hoạt Jockey Independence Check — Hệ thống thực thi tự động (Node 4.4)] ↓ Kết quả: Pass ↓ tiếp tục | Fail ↓ Disqualified khẩn cấp [Xác nhận danh sách xuất phát chính thức]`

Nếu Doctor xác minh sai lệch danh tính, Doctor kết luận Unfit, hoặc hệ thống phát hiện Jockey vi phạm quan hệ gia đình ruột thịt với Horse Owner đối thủ, cặp Ngựa–Jockey bị chuyển trạng thái Disqualified, danh sách xuất phát được cập nhật, ba thông báo khẩn gửi đến Horse Owner, Jockey và Admin, đồng thời điểm ảo dự đoán liên quan được hoàn trả cho Spectator. **(EC-26 — BR-30)** Toàn bộ chuỗi loại khẩn cấp này phải nằm trong **1 ACID transaction duy nhất**; lỗi bất kỳ bước nào → ROLLBACK toàn bộ, `RaceEntry` giữ nguyên trạng thái trước đó. **(EC-32)** Khi cặp bị DQ đã đóng lệ phí, transaction tự động chuyển `EntryFeeStatus: Paid → Refund Pending` (hoàn lệ phí thật của Owner).

> **Phân công & COI Doctor (EC-38 — BR-47):** Doctor chỉ được cân/xác minh danh tính/kết luận `Fit/Unfit` cho Race nếu đã được Admin phân công (`DoctorAssignments`) và vượt **Doctor COI Check** (không là người thân Owner trong race) — đối xứng ràng buộc COI của Referee. **(EC-39)** Ngưỡng cảnh báo cân nặng đọc từ cấu hình giải (`PreRaceWeightThresholdKg`/`PostRaceWeightDiffThresholdKg`); baseline `SelfDeclaredWeight` bắt buộc NOT NULL.

---

### **2.2.6 Pha 5: Theo Dõi Diễn Biến Cuộc Đua (Live Race Event)**

**Mục đích:** Race Referee chuyển cuộc đua sang Live, Spectator theo dõi animation mô phỏng, Race Referee ghi vi phạm và xử lý khiếu nại trước khi chốt kết quả sơ bộ.

**Tính chất bắt buộc của Module H:** Live Race là **animation giả lập phía client**. Mỗi ngựa được biểu diễn bằng chip đồ họa trên Canvas/SVG; vị trí chip thay đổi bằng Math.random() và setInterval(100ms). Animation không dùng WebSockets, SSE hoặc GPS thực tế. Khi Referee xác nhận kết quả sơ bộ, chip ngựa được đẩy về đúng thứ hạng thực tế tại vạch đích.

> **Tiền điều kiện chuyển `Live` (EC-17/EC-24 — BR-22):** Hệ thống auto-cancel **đồng bộ** mọi `RaceEntry` còn `Pending` (không chờ scheduler) và xác nhận mọi entry hợp lệ đã `Confirmed` + `PreRaceJockeyWeight` + `Fit` + danh tính khớp + Independence Check đạt; thiếu bất kỳ điều kiện → chặn chuyển `Live`.

> **Cửa sổ Protest (EC-20/EC-27 — BR-16):** Protest chỉ nhận khi Race `Unofficial`, biên bản chưa khóa, trong thời hạn cấu hình (mặc định 2 giờ); giới hạn số lần khiếu nại lại. **(EC-43/EC-44 — BR-52/BR-53)** Người nộp phải là Owner/Jockey của cặp đấu hợp lệ trong chính race đó; entry bị khiếu nại cùng race. **(EC-41 — BR-50)** Mỗi lần Approve Protest điều chỉnh thứ hạng → chạy lại validation toàn vẹn standard-ranking trước khi cho Declare Official.

> **Cưỡng chế Weigh-Out trước khi chốt (EC-42 — BR-51):** Mọi cặp hợp lệ phải có `PostRaceJockeyWeight` NOT NULL; thiếu → chặn Referee chốt biên bản (Node 5.6), đối xứng với cách chặn khi thiếu `FinishPosition`.

`[Race Referee chuyển Race.Status: Upcoming → Live] ↓ Hệ thống khóa Prediction Gate [Spectator xem Live Simulation Animation] ↓ [Race Referee ghi nhận vi phạm bằng dropdown mã lỗi] ↓ [Horse Owner/Jockey có thể nộp Protest trong cửa sổ hợp lệ] ↓ [Doctor cân Jockey sau đua — Post-Race Weigh-Out (Node 5.1-WO)] ↓ Ghi PostRaceJockeyWeight; nếu chênh lệch Pre/Post vượt ngưỡng → Flag cảnh báo cho Referee ↓ Referee quyết định có ghi vào Violations hay không [Race Referee xử lý Protest và hình phạt] ↓ [Race Referee xác nhận thứ hạng sơ bộ] Trạng thái: Unofficial`

---

### **2.2.7 Pha 6: Công Bố Kết Quả & Tính Thưởng (Results & Disbursement)**

**Mục đích:** Admin công bố kết quả chính thức bằng một ACID Transaction duy nhất; nếu bất kỳ bước nào lỗi, toàn bộ transaction phải ROLLBACK.

**ACID Transaction gồm đúng 6 bước:**

1. Chuyển trạng thái cuộc đua Race.Status = 'Official'.
2. Khóa cứng biên bản thi đấu điện tử RaceReport.IsLocked = true.
3. Tự động đối chiếu dự đoán và trả thưởng ví ảo Spectator.
4. Tự động cập nhật Bảng xếp hạng Ngựa và Jockey Standings.
5. Tính toán và phân bổ tiền thưởng quỹ Purse theo lý thuyết.
6. Ghi nhận nhật ký kiểm toán hệ thống (AuditLog).

\`\[Admin bấm Declare Official\] ↓ trong 1 Database Transaction duy nhất \[6 bước ACID\] ↓ nếu thành công \[Official Result + Locked RaceReport + Prediction Reward + Leaderboard + Purse Allocation + AuditLog\] ↓ \[Báo cáo / CSV / PDF / In trực tiếp từ trình duyệt\]

Nếu lỗi ở bất kỳ bước nào: ROLLBACK toàn bộ → Race giữ trạng thái Unofficial → RaceReport chưa khóa → Không cập nhật ví ảo, Leaderboard, Purse, AuditLog\`

Tiền thưởng thực tế được xử lý ngoài HRTMS. Admin chỉ cập nhật trạng thái Paid hoặc Unpaid trong hệ thống sau khi đơn vị tổ chức hoàn tất chi trả ngoài đời thực; thao tác này được ghi Audit Log.

**Các quy tắc bổ sung trong transaction Declare Official (đồng bộ INTRODUCTION):** **(EC-03 — BR-29)** Bước 3 bao gồm hoàn điểm (`Refunded`) cho mọi Prediction còn `Pending` trên cặp Disqualified/Cancelled — kể cả ngựa bị truất quyền do Protest ở Pha 5 — không để dự đoán kẹt chưa quyết toán. **(EC-08/EC-33 — BR-26/BR-42)** Bước 5 phân bổ Purse theo tỷ lệ nạp từ bảng cấu hình `PrizeDistributions` của giải (tổng bắt buộc 100%, validate khi lưu); giải chưa cấu hình đủ tỷ lệ → **chặn Declare Official**. Khi số ngựa về đích hợp lệ ít hơn số vị trí thưởng, phần dư ghi nhận `Remainder` để Admin xử lý thủ công. **(EC-31 — BR-28)** Mọi truy vấn đọc Leaderboard/ví dùng tối thiểu `READ COMMITTED`; sau khi transaction commit, hệ thống phát Notification kích hoạt re-fetch để Spectator không thấy trạng thái trung gian trong chu kỳ polling 30 giây.

---

### **2.2.8 Swimlane Diagram — Cấu Trúc Toàn Luồng**

`Admin : Tạo giải → Duyệt hồ sơ → Lập lịch → Phân công Referee & Doctor → Declare Official → Báo cáo Horse Owner : Khai báo ngựa → Đăng ký giải → Mời Jockey → Xác nhận lịch → Protest nếu có Jockey : Khai báo chứng chỉ → Nhận/Từ chối lời mời → Xem lịch → Protest nếu có Race Referee : Nhận phân công → Kích hoạt Independence Check → Chuyển Live → Ghi vi phạm → Xử lý Protest → Chốt sơ bộ Doctor : Pre-Race Weigh-In → Xác minh danh tính ngựa Paddock → Khám lâm sàng Fit/Unfit → Post-Race Weigh-Out Spectator : Xem giải/lịch → Đặt Win → Xem Live Animation → Nhận điểm ảo nếu đúng HRTMS : RBAC → Validation → COI Check → Prediction Gate → ACID Transaction → Audit Log → Notification`

---

### **2.2.9 Phân Tích Happy Path và Unhappy Cases**

| **Nhóm lỗi** | **Điểm phát sinh** | **Kết quả xử lý** |
| --- | --- | --- |
| Hồ sơ Jockey bị từ chối | Module D | Không xuất hiện trong danh sách tìm kiếm để ghép cặp |
| Hồ sơ ngựa bị từ chối hoặc auto-reject | Module C | Không được đưa vào lập lịch; lý do từ chối bắt buộc khi Admin reject |
| Withdrawal / quá hạn xác nhận | Module E | RaceEntry.Cancelled, PostPosition.Vacant, thông báo khẩn cho Admin, ghi Audit Log |
| Xung đột lợi ích trọng tài | Module F | Chặn phân công và yêu cầu Admin chọn trọng tài khác |
| Unfit / sai danh tính / Jockey không độc lập | Module G | Disqualified, cập nhật danh sách chạy, hoàn điểm ảo dự đoán liên quan |
| Protest được chấp thuận | Module I | Áp dụng hình phạt, tính lại thứ hạng, thông báo closed-loop cho hai bên |
| Lỗi Declare Official | Module J | ROLLBACK toàn bộ ACID Transaction |
...
| RefereeProfiles/DoctorProfiles.Status (EC-37) | Khởi tạo `Pending`; Admin duyệt onboarding → `Active`; chỉ `Active` mới được phân công |
...
| Open Registration | Xanh dương | Đang mở đăng ký |
| Closed Registration | Xám đậm | Đã đóng đăng ký |
| Pre-Race | Vàng | Hoàn tất lập lịch & phân công |
| In-Progress | Đỏ | Race đầu tiên đã Live |
| Completed | Xanh lá | Mọi Race đã Official/Cancelled |
...
| UI-S21 | Registration & Pairing | Đăng ký race; tìm & mời Jockey | Horse Owner | C, D |
...
Ràng buộc: Nút **Approve khóa khi `EntryFeeStatus ≠ Paid`** (Node 2.5, UI-PR6). Auto-reject khi `Horse.Breed ≠ Tournament.AllowedBreed` (Admin không override). Reject bắt buộc lý do ≥10 ký tự → gửi Notification kèm lý do. Tab onboarding: đối chiếu CertificationLevel/MedicalLicenseNumber → `Pending → Active` (EC-37).
...
**UI-S21 — Race Registration & Pairing**
Mục đích: Đăng ký ngựa vào race + ghép cặp Jockey. Actor: Horse Owner.
Thành phần: Chọn ngựa (Approved) + chọn Race; danh sách Jockey `Active` (search) với cột "Đạt MinExp?"; nút Gửi lời mời; trạng thái Pairing + EntryFee.
Ràng buộc: Kiểm tra `ExperienceYears ≥ MinJockeyExperienceYears` **tái kiểm tra khi vào RaceEntry** (EC-21). Chặn 1 ngựa/Jockey trùng trong cùng race (EC-40); chặn vượt `MaxHorses` (EC-46). EntryFee tạo tự động `Unpaid` (hoặc `Paid` nếu phí=0).
...
Mục 3.2 đặc tả toàn bộ **yêu cầu chức năng (Functional Requirements — FR)** của hệ thống HRTMS Phase 1. Nội dung được suy luận trực tiếp (traceability) từ **SRS Chương 1 (Introduction — Scope, 17 Module A–Q)**, **SRS Chương 2 (Overall Description)** — đặc biệt mục **2.3.2 Danh sách 17 Module Chức năng (đóng vai trò Product Functions)**, mục **2.2 Main Flows (6 pha nghiệp vụ)** — và **Database Schema Specification** (26 bảng Logical ERD). Mỗi FR được truy xuất ngược tới Module nghiệp vụ, màn hình giao diện (mục 3.1), thực thể dữ liệu và Business Rule (BR-01…BR-57) / Edge-case (EC-01…EC-48) liên quan.

Mỗi **Product Function (PF)** = một Module A–Q. Mỗi PF được mở rộng (expand) thành nhiều FR chi tiết. Tổng quan ánh xạ PF → FR tại mục 3.2.2; chi tiết từng FR tại mục 3.2.3–3.2.19; ma trận truy vết đầy đủ tại mục 3.2.20.
...
| --- | --- | --- | --- |
| A | ACC | Quản lý Tài khoản & Phân quyền | 9 |
| B | TRN | Quản lý Giải đấu | 10 |
| C | HRS | Đăng ký Ngựa & Duyệt hồ sơ | 8 |
...
- **Tiêu chí chấp nhận:**
  1. Mỗi vai trò hiển thị đúng tập trường bắt buộc của vai trò đó ở bước B2.
  2. Email là duy nhất toàn hệ thống; trùng email → chặn và báo lỗi tiếng Việt.
  3. Jockey và Race Referee bắt buộc khai báo quan hệ gia đình ở bước B2 (phục vụ Independence/COI Check).
  4. Đăng ký thành công tạo bản ghi `Users` + bản ghi `*Profiles` tương ứng.
...
#### REQ-F-ACC.7 — Onboarding Trọng tài (Pending → Active)

- **Priority:** High
- **Mô tả:** Hệ thống PHẢI khởi tạo hồ sơ Race Referee ở trạng thái `Pending` và yêu cầu Admin duyệt onboarding (đối chiếu chứng chỉ trọng tài) để chuyển sang `Active`. Chỉ tài khoản `Active` mới được phân công nhiệm vụ.
- **Tiêu chí chấp nhận:**
  1. Referee `Pending` không xuất hiện trong danh sách phân công (Module F).
  2. Duyệt onboarding ghi `AuditLogs` (`Approve_Referee`).
- **Actor:** Admin, Referee | **DB:** RefereeProfiles, AuditLogs | **UI:** UI-S10 | **Truy vết:** BR-46; EC-37.

#### REQ-F-ACC.8 — Onboarding Bác sĩ thú y (Pending → Active)

- **Priority:** High
- **Mô tả:** Hệ thống PHẢI khởi tạo hồ sơ Doctor ở trạng thái `Pending` và yêu cầu Admin duyệt onboarding (đối chiếu giấy phép hành nghề) để chuyển sang `Active`. Chỉ Doctor `Active` mới được phân công cho cuộc đua.
- **Tiêu chí chấp nhận:**
  1. Doctor `Pending` không xuất hiện trong danh sách phân công Doctor (Module G).
  2. Duyệt onboarding ghi `AuditLogs` (`Approve_Doctor`).
- **Actor:** Admin, Doctor | **DB:** DoctorProfiles, AuditLogs | **UI:** UI-S10 | **Truy vết:** BR-46; EC-37.
...
- **Priority:** High
- **Mô tả:** Hệ thống PHẢI quản lý `Tournaments.Status` theo state machine một chiều: `Draft → Open Registration → Closed Registration → Pre-Race → In-Progress → Completed`, mỗi chuyển tiếp có tác nhân/điều kiện rõ ràng và được ghi `AuditLogs`.
- **Tiêu chí chấp nhận:**
  1. Chuyển trạng thái sai trình tự → bị chặn.
  2. Trạng thái cấp giải độc lập với trạng thái cấp Race.
  3. Mỗi chuyển tiếp ghi nhật ký kiểm toán.
...
- **Priority:** High
- **Mô tả:** Hệ thống PHẢI tự động đối chiếu và từ chối (auto-reject) hồ sơ khi `Horse.Breed ≠ Tournament.AllowedBreed` HOẶC `DopingTestResult = Failed`; Admin KHÔNG được override.
- **Tiêu chí chấp nhận:**
...
- **Priority:** High
- **Mô tả:** Khi Owner đăng ký ngựa vào cuộc đua, hệ thống PHẢI tạo `RaceEntry` với `EntryFeeStatus = 'Unpaid'` (hoặc tự động `Paid` nếu `EntryFeeAmount = 0`). Admin xác nhận đã nhận phí (ngoài hệ thống) rồi cập nhật `Paid`, ghi `EntryFeeConfirmedBy/At` và `AuditLogs` (`Update_Entry_Fee_Status`).
- **Tiêu chí chấp nhận:**
...
- **Priority:** Medium
- **Mô tả:** Hệ thống PHẢI cho phép Horse Owner tìm kiếm và lọc danh sách Jockey ở trạng thái `Active`.
- **Tiêu chí chấp nhận:**
  1. Chỉ hiển thị Jockey `Active`.
  2. Hỗ trợ lọc theo kinh nghiệm/tiêu chí liên quan.
- **Actor:** Horse Owner | **DB:** JockeyProfiles | **UI:** UI-S21 | **Truy vết:** Module D.

#### REQ-F-JOC.5 — Gửi lời mời thuê Jockey

- **Priority:** High
- **Mô tả:** Hệ thống PHẢI cho phép Horse Owner gửi lời mời thuê Jockey để tạo quan hệ ghép cặp (Pairing).
- **Tiêu chí chấp nhận:**
  1. Chỉ mời được Jockey `Active`.
  2. Lời mời tạo bản ghi Pairing ở trạng thái chờ phản hồi.
- **Actor:** Horse Owner | **DB:** Pairings | **UI:** UI-S21 | **Truy vết:** Module D.
...
- **Tiêu chí chấp nhận:**
  ...
  2. Vì Pairing độc lập với giải, ràng buộc được re-check khi tạo RaceEntry.
...
- **Priority:** High
- **Mô tả:** Trước khi phân công, hệ thống PHẢI tự động kiểm tra: Trọng tài không được là người thân trực hệ (cha, mẹ, vợ, chồng, con) của bất kỳ Horse Owner nào có ngựa trong cuộc đua đó.
- **Tiêu chí chấp nhận:**
  1. Phát hiện xung đột → chặn phân công, yêu cầu chọn trọng tài khác.
  2. Trọng tài tích cam kết COI khi khai báo.
...
- **Priority:** High
- **Mô tả:** Hệ thống PHẢI yêu cầu Admin phân công Doctor tường minh cho từng cuộc đua (`DoctorAssignments`) và kiểm tra COI: Doctor không được khám cuộc đua có ngựa của người thân trực hệ. Backend chỉ chấp nhận thao tác cân/khám/Unfit của Doctor đã được phân công cho Race đó.
- **Tiêu chí chấp nhận:**
  1. Doctor chưa phân công thao tác trên Race → từ chối tại Backend.
  2. Doctor có xung đột nhân thân → chặn phân công; thay đổi sau phân công → re-run COI.
...
- **Priority:** High
- **Mô tả:** Hệ thống PHẢI tự động xác minh (do Race Referee kích hoạt) rằng Jockey điều khiển ngựa có chứng chỉ hợp lệ và **không có quan hệ gia đình ruột thịt** với bất kỳ Horse Owner đối thủ nào có ngựa trong cùng cuộc đua.
- **Tiêu chí chấp nhận:**
  1. Phát hiện vi phạm tính độc lập → kích hoạt Emergency Disqualification.
  2. Việc kích hoạt thuộc Race Referee; hệ thống thực thi tự động (Separation of Concerns).
...
- **Priority:** High
- **Mô tả:** Hệ thống PHẢI cho phép Spectator đặt dự đoán Win, trừ điểm ví và ghi đồng thời một dòng `VirtualPointsTransactions` loại `Prediction Placed` (âm) trong cùng giao dịch, giữ bất biến `Balance = SUM(giao dịch)`.
- **Tiêu chí chấp nhận:**
...
**Quy tắc bao phủ (Coverage):** Mỗi Product Function (Module A–Q) ánh xạ tới ≥ 3 FR; mỗi FR ánh xạ tới ≥ 1 màn hình UI (3.1) và ≥ 1 bảng dữ liệu (3.4); mỗi FR có ràng buộc phi chức năng được phản ánh ở ≥ 1 NFR (3.3). Toàn bộ Business Rule BR-01…BR-57 và Edge-case EC-01…EC-48 đều được truy vết qua trường "Truy vết" của các FR tương ứng.

---

*Mục 3.2 — FUNCTIONAL REQUIREMENTS kết thúc. Tổng: 17 Module Product Function → 109 Functional Requirements.*
...
Tổng cộng **25 bảng** chia thành 6 nhóm phân hệ. Dưới đây là danh sách đầy đủ.
...
**Bảng: Users**
- Attributes: 11 (UserId, Username, FullName, Email, PasswordHash, Role, Status, FailedLoginAttempts, LockoutEnd, CreatedAt, UpdatedAt)
- PK: UserId (INT, Identity)
- FKs: không có FK đến bảng khác (bảng gốc trung tâm)
- Key constraints: UNIQUE(Username), UNIQUE(Email), CHECK Role IN (6 giá trị), CHECK Status IN (3 giá trị)
...
**Bảng: OwnerProfiles**
- Attributes: 5 (OwnerId, PhoneNumber, IdentityNumber, CreatedAt, UpdatedAt)
- PK: OwnerId = FK → Users.UserId (1:1 extension)
- Key constraints: UNIQUE(IdentityNumber)
...
- Key constraints: CHECK Status IN (Pending/Active/Suspended)
...
- Key constraints: UNIQUE(MedicalLicenseNumber), CHECK Status IN (Pending/Active/Suspended)
...
- Key constraints: UNIQUE(RoundId, RaceNumber); CHECK Status IN (Upcoming/Live/Unofficial/Official/Cancelled)
...
**Bảng: Pairings**
- Attributes: 8 (PairingId, HorseId, JockeyId, Status, RequestMessage, ResponseReason, CreatedAt, UpdatedAt)
- PK: PairingId (INT, Identity)
- FKs: HorseId → Horses.HorseId; JockeyId → JockeyProfiles.JockeyId
- Key constraints: CHECK Status IN (Pending/Accepted/Declined)
...
#### NHÓM 5 — Predictions & Virtual Wallets (Module M, N)
...
- Key constraints: CHECK Type IN (SignUp Bonus/Prediction Win Reward/Prediction Refund/Prediction Placed); Amount âm khi Prediction Placed, dương khi còn lại
...
1. **Kích thước schema:** 25 bảng trong 1 Logical ERD duy nhất sẽ cực kỳ khó đọc — diagram sẽ rất rộng, đặc biệt khi mỗi bảng có đến 22 attributes (RaceEntries).
...
| Pairings | REQ-F-JOC.5 (gửi lời mời), REQ-F-JOC.6 (phản hồi ghép cặp), REQ-F-JOC.7 (check kinh nghiệm) | UI-S05 (Pairing Request), UI-S06 (Horse Detail — invite Jockey) |
...
1. **PK Auto-increment:** 22/25 bảng dùng INT Identity PK — không cần app tạo ID.
...
- **3.4 Conceptual & Logical ERD** + **`database_design_guide.md` v2.1** — nguồn schema chuẩn (25 bảng, 6 nhóm phân hệ, mục 3.4.3.1–3.4.3.6);
...
Bảng tổng hợp toàn bộ 25 bảng (chi tiết cấp cột: xem guide 3.4.3 — canonical). Cột **Độ nhạy cảm** ánh xạ phân loại tại mục 3.6.6.
...
| 13 | 3 | `Pairings` | 8 | PairingId (Identity) | CHECK Status | Internal |
...
> **Tổng:** 25 bảng · 22 bảng dùng `INT Identity` PK · 3 junction tables (`RaceEntries`, `RefereeAssignments`, `DoctorAssignments`) · 5 bảng mở rộng 1:1 từ `Users`.
...
| `Tournaments.Status` | `Draft`/`Open Registration`/`Closed Registration`/`Pre-Race`/`In-Progress`/`Completed` | State machine cấp giải (EC-36) |
| `Rounds.Status` | `Upcoming`/`In-Progress`/`Completed`/`Cancelled` | Trạng thái vòng |
| `Races.Status` | `Upcoming`/`Live`/`Unofficial`/`Official`/`Cancelled` | State machine cấp cuộc đua (độc lập trạng thái giải) |
...
| `Pairings.Status` | `Pending`/`Accepted`/`Declined` | Trạng thái lời mời ghép cặp |
...
| `VirtualPointsTransactions.Type` | `SignUp Bonus` | +1000 khi tạo ví (EC-47) |
...
│ Referee: Violations,│      │  SQL Server 2022 (25 bảng)   │     │   (outbound only)    │   Gateway
...
| --- | --- | --- | --- | --- |
| DICT.1–4 (từ điển, enum) | Toàn bộ 25 bảng | REL.2 | DB.1/DB.2 | Toàn bộ FR |
...
*Mục 3.6 — DATA REQUIREMENTS kết thúc. Tổng: 9 nhóm yêu cầu dữ liệu phủ 25 bảng, với các mục `[CẦN CHỐT]` tổng hợp tại 3.6.13.*
...
Phụ lục A đặc tả toàn bộ **Use Case (UC)** của hệ thống HRTMS Phase 1, suy luận trực tiếp (traceability) từ: **6 Actor (SRS 2.3.1)**, **17 Module Product Function A–Q (SRS 2.3.2)**, **109 Functional Requirements (3.2)**, **35 màn hình UI-S01…UI-S35 (3.1)**, **26 bảng Logical ERD (3.4)** và **NFR (3.3)**. Mỗi UC được truy xuất ngược tới FR (`REQ-F-*`), màn hình (`UI-S*`), thực thể dữ liệu, Business Rule (`BR-01…BR-57`), Edge-case (`EC-01…EC-48`) và NFR (`NF-*`) liên quan.
...
Tổng **49 UC**, nhóm theo 6 Pha. Mã UC: `UC-<nn>`. (Chi tiết comprehensive tại A.4.)

#### A.2.1 — Pha 1: Account & Tournament Setup (12 UC)

| UC | Tên | Primary Actor | Secondary/Participating | FR chính | UI |
| --- | --- | --- | --- | --- | --- |
| UC-01 | Tự đăng ký tài khoản | Owner/Jockey/Referee/Doctor/Spectator | System, SMTP | ACC.1, ACC.9 | UI-S02 |
| UC-02 | Đăng nhập & RBAC Routing | Tất cả (người dùng) | System | ACC.5, ACC.6, SEC.2 | UI-S01 |
| UC-03 | Quản lý hồ sơ cá nhân & đổi mật khẩu | Tất cả (người dùng) | System | ACC.3 | UI-S03 |
...
| UC-06 | Duyệt onboarding Referee/Doctor | Admin | Referee, Doctor, SMTP | ACC.7, ACC.8 | UI-S10 |
| UC-07 | Tạo & cấu hình giải đấu | Admin | System | TRN.1, TRN.2, TRN.3, TRN.5 | UI-S09 |
| UC-08 | Cấu hình tỷ lệ chia thưởng | Admin | System | TRN.4 | UI-S09 |
| UC-09 | Dựng cấu trúc Round/Race & quỹ thưởng | Admin | System | TRN.6, TRN.7, PRZ.1, PRZ.5 | UI-S09 |
| UC-10 | Quản lý trạng thái giải (state machine) | Admin | System | TRN.8 | UI-S08, UI-S09 |
| UC-11 | Cập nhật giải trước khi bắt đầu | Admin | System | TRN.9 | UI-S09 |
| UC-12 | Hủy giải đấu (Cancellation Flow) | Admin | System, SMTP | TRN.10, HRS.8 | UI-S09 |

#### A.2.2 — Pha 2: Registration — Horse & Jockey (8 UC)

| UC | Tên | Primary Actor | Secondary/Participating | FR chính | UI |
| --- | --- | --- | --- | --- | --- |
| UC-13 | Khai báo & quản lý hồ sơ ngựa | Horse Owner | System | HRS.1, HRS.2, HRS.3, HRS.6 | UI-S20 |
| UC-14 | Đăng ký ngựa vào cuộc đua | Horse Owner | System | HRS.7, HRS.4, SCH.7, SCH.8 | UI-S21 |
| UC-15 | Duyệt/từ chối hồ sơ ngựa | Admin | System, SMTP | HRS.5, HRS.4 | UI-S10 |
| UC-16 | Xác nhận lệ phí Entry Fee | Admin | System | HRS.7, HRS.8 | UI-S12 |
| UC-17 | Khai báo hồ sơ Jockey & quan hệ gia đình | Jockey | System | JOC.1, JOC.2 | UI-S25 |
| UC-18 | Duyệt/từ chối hồ sơ Jockey | Admin | System, SMTP | JOC.3 | UI-S10 |
| UC-19 | Tìm & mời thuê Jockey | Horse Owner | System, Jockey | JOC.4, JOC.5, JOC.7 | UI-S21 |
| UC-20 | Phản hồi lời mời & xác nhận ghép cặp | Jockey | Horse Owner, System | JOC.6 | UI-S26, UI-S21 |

#### A.2.3 — Pha 3: Scheduling, Draw & Prediction Gate (7 UC)

| UC | Tên | Primary Actor | Secondary/Participating | FR chính | UI |
| --- | --- | --- | --- | --- | --- |
| UC-21 | Phân bổ cặp vào cuộc đua | Admin | System | SCH.1, SCH.6, SCH.8 | UI-S09 |
| UC-22 | Bốc thăm vị trí xuất phát | Admin | System | SCH.2 | UI-S09 |
| UC-23 | Xác nhận tham gia trước Cut-off | Horse Owner | System | SCH.4 | UI-S22 |
| UC-24 | Rút lui / quá hạn (Withdrawal Flow) | Horse Owner | System, SMTP | SCH.5, HRS.8 | UI-S22 |
| UC-25 | Phân công Trọng tài & COI Check | Admin | System, Referee, SMTP | REF.1, REF.2, REF.3, REF.4, REF.5 | UI-S11 |
| UC-26 | Cấu hình & mở/đóng cổng dự đoán | Admin | System | PRD.1, PRD.2 | UI-S15 |
| UC-27 | Xem lịch công khai & Tournament Hub | Tất cả (người dùng) | System | SCH.3 | UI-S05 |

#### A.2.4 — Pha 4: Pre-Race Medical Check (7 UC)

| UC | Tên | Primary Actor | Secondary/Participating | FR chính | UI |
| --- | --- | --- | --- | --- | --- |
| UC-28 | Phân công Doctor & Doctor COI Check | Admin | System | MED.1 | UI-S11 |
| UC-29 | Cân Jockey trước đua (Pre-Race Weigh-In) | Doctor | System | MED.2, MED.3 | UI-S31 |
| UC-30 | Xác minh danh tính ngựa tại Paddock | Doctor | System | MED.4 | UI-S31 |
| UC-31 | Khám lâm sàng Fit/Unfit | Doctor | System | MED.5 | UI-S31 |
| UC-32 | Kích hoạt Jockey Independence Check | Race Referee | System | MED.6 | UI-S28, UI-S31 |
| UC-33 | Loại ngựa khẩn cấp (Emergency Disqualification) | System | Doctor/Referee, SMTP | MED.7, HRS.8 | UI-S31 |
| UC-34 | Xác nhận danh sách xuất phát chính thức | Race Referee | System | MED.8 | UI-S28 |

#### A.2.5 — Pha 5: Live Race & Protest (8 UC)

| UC | Tên | Primary Actor | Secondary/Participating | FR chính | UI |
| --- | --- | --- | --- | --- | --- |
| UC-35 | Chuyển cuộc đua sang Live | Race Referee | System | RACE.1, RACE.2 | UI-S28 |
| UC-36 | Xem Live Race Animation | Spectator | System | RACE.3 | UI-S07 |
| UC-37 | Ghi nhận vi phạm | Race Referee | System | RACE.4, RACE.5 | UI-S28 |
| UC-38 | Cân Jockey sau đua (Post-Race Weigh-Out) | Doctor | System | RACE.7 | UI-S31 |
| UC-39 | Chốt kết quả sơ bộ (Unofficial) | Race Referee | System | RACE.8 | UI-S28 |
| UC-40 | Nộp khiếu nại (Protest) | Horse Owner / Jockey | System | RACE.6, PRT.1, PRT.2 | UI-S23 |
| UC-41 | Điều tra & phán quyết Protest | Race Referee | System, SMTP | PRT.3, PRT.4, PRT.5 | UI-S29 |
| UC-42 | Lập & khóa biên bản thi đấu | Race Referee | System | PRT.6, PRT.7 | UI-S29 |

#### A.2.6 — Pha 6: Declare Official, Reward & Leaderboard (7 UC)

| UC | Tên | Primary Actor | Secondary/Participating | FR chính | UI |
| --- | --- | --- | --- | --- | --- |
| UC-43 | Đặt dự đoán Win | Spectator | System | PRD.3, PRD.4, PRD.5, PRD.6 | UI-S33 |
| UC-44 | Công bố kết quả chính thức (Declare Official) | Admin | System, SMTP | RES.1–RES.5, PRZ.2–PRZ.4, REC.1, REC.2, LDR.* | UI-S13 |
| UC-45 | Quản lý phân bổ & chi trả thưởng (Purse) | Admin | System | PRZ.3, PRZ.4, PRZ.6 | UI-S14 |
| UC-46 | Xem Bảng xếp hạng & Standings | Tất cả (người dùng) | System | LDR.1, LDR.2, LDR.3, LDR.4 | UI-S06 |
| UC-47 | Xem đối chiếu dự đoán & ví (Reconciliation) | Spectator | System | REC.3, REC.4, REC.5 | UI-S34, UI-S35 |
| UC-48 | Nhận thông báo (In-app + Email) | Tất cả (người dùng) | System, SMTP | NOTI.1, NOTI.2, NOTI.3 | UI-S04 |
| UC-49 | Kết xuất báo cáo (CSV/PDF/in) | Theo quyền (RBAC) | System | RPT.1, RPT.2, RPT.3 | UI-S18 |
...
rectangle "HRTMS Phase 1" {
  usecase "Pha 1: Account &\nTournament Setup" as P1
  usecase "Pha 2: Registration\n(Horse & Jockey)" as P2
  usecase "Pha 3: Scheduling,\nDraw & Prediction Gate" as P3
  usecase "Pha 4: Pre-Race\nMedical Check" as P4
  usecase "Pha 5: Live Race\n& Protest" as P5
  usecase "Pha 6: Declare Official,\nReward & Leaderboard" as P6
}
...
#### A.3.1 — Pha 1: Account & Tournament Setup
...
#### A.3.2 — Pha 2: Registration (Horse & Jockey)
...
#### A.3.3 — Pha 3: Scheduling, Draw & Prediction Gate
...
#### A.3.4 — Pha 4: Pre-Race Medical Check
...
#### A.3.6 — Pha 6: Declare Official, Reward & Leaderboard
...
| UC-06 | Duyệt onboarding Referee / Doctor | 1 | Admin | REQ-F-ACC.7, ACC.8 |
...
| UC-14 | Đăng ký ngựa vào cuộc đua (RaceEntry) | 2 | Horse Owner | REQ-F-HRS.3, HRS.4 |
...
| UC-19 | Tìm & mời thuê Jockey | 2 | Horse Owner | REQ-F-JOC.4, JOC.5 |
...
| UC-43 | Đặt dự đoán Win | 6 | Spectator | REQ-F-PRD.2, PRD.3 |
...
| UC-47 | Xem đối chiếu dự đoán & ví (Reconciliation) | 6 | Spectator | REQ-F-PRD.4, PRD.5 |
...
#### A.5.1 — UC → Functional Requirements (bao phủ đủ 109 FR)
...
> **Kết luận bao phủ:** 109/109 FR được trace bởi ≥ 1 UC. FR mang tính nền tảng/xuyên suốt (SEC.4 RBAC, SEC.6 audit immutability) được hiện thực như **ràng buộc chung** áp cho mọi UC thay vì một UC riêng (đúng Decision 1=B).
...
- **Diagram Pha 1 (A.3.1):** Actor `Owner/Jockey/Referee/Doctor/Spectator` nối UC-01/02/03; Admin nối UC-04…12. `«include»`: UC-01 → *Khởi tạo ví + SignUp Bonus*; UC-05/07/12 → *Ghi AuditLog*. `«extend»`: UC-08, UC-09 mở rộng UC-07 (cấu hình tùy chọn của giải). System tham gia UC-12 (giao dịch hủy); SMTP nhận từ UC-06, UC-12.

- **Diagram Pha 2 (A.3.2):** Owner nối UC-13/14/19; Jockey nối UC-17/20; Admin nối UC-15/16/18. `«include»`: UC-14 & UC-15 → *Auto-reject (Breed/Doping)*; UC-19 & UC-14 → *Re-check kinh nghiệm tối thiểu*. `«extend»`: UC-13 → UC-15 (sửa trường nhạy cảm sau Approved kích hoạt duyệt lại). System primary cho *Auto-reject*; SMTP nhận từ UC-15, UC-18.
...
- Thuật ngữ & tên thực thể đồng nhất với 1.3 Definitions, 3.4 ERD (26 bảng): `RaceEntries`, `Pairings`, `PursePayouts`, `VirtualPointsTransactions`, `FamilyRelationshipDeclarations`…
...
| --- | --- | --- |
| D1 | Quy tắc tách UC | **B** — Gộp theo *user-goal*; FR validation/transaction/guard → business rule + alternate/exception flow. **49 UC** truy vết đủ 109 FR |
...
| **Pairing** | Cặp ghép Ngựa–Jockey; độc lập với giải, `Confirmed` sau khi Owner xác nhận cuối. |
...
| **BR-`<nn>`** | Business Rule (BR-01…BR-57). |
