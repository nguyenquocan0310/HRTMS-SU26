...
- Dòng đầu là header; thứ tự cột **cố định**. RBAC chủ yếu lọc **dòng**; ngoại lệ duy nhất:
  `entry-list` của **Spectator** dùng bộ cột rút gọn (xem mục 2).
- Kết thúc dòng bằng `\r\n`.
- Escape theo RFC 4180: field chứa `,` `"` `\n` hoặc `\r` được bọc trong `"`; dấu `"` bên trong nhân đôi thành `""`.
- **Chống CSV formula injection:** field bắt đầu bằng `=` `+` `-` `@` tab CR được prefix `'` (apostrophe)
  trước khi escape — trừ giá trị parse được thành số (vd `-5.00` của cột tiền) thì giữ nguyên.
...
**Riêng `entry-list` với role Spectator** (quyết định nhóm 2026-07-11): bỏ hẳn 3 cột trạng thái nội bộ
`PairingStatus`, `EntryFeeStatus`, `EnrollmentApprovalStatus` khỏi cả header lẫn row (`OwnerName` giữ lại):

```
TournamentId, TournamentName, RoundName, RaceNumber, HorseId, HorseName, OwnerName, JockeyName, PairingId, EntryStatus, IsWithdrawn
```
...
Áp dụng **cùng** validate + RBAC filter như endpoint export (kể cả bộ cột rút gọn của Spectator ở `entry-list`).

> **Quyết định audit cho JSON path (cần nhóm xác nhận):** hiện tại endpoint JSON **không** ghi audit —
> chỉ export CSV ghi `Export_Report` (cả `success` lẫn `denied`, mục 5). Nếu nhóm coi print-view tương đương
> export (người dùng vẫn lấy được toàn bộ dữ liệu), cần chốt để bổ sung action `View_Report` cho request
> hợp lệ và bản ghi `denied` cho request bị từ chối quyền ở JSON path. Chưa chốt → giữ nguyên hành vi này,
> không tự tạo audit policy mới.
