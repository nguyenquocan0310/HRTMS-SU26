...
const friendlyError = (error: unknown) =>
  error instanceof Error && error.message
    ? error.message
    : "Không thể bốc thăm vị trí xuất phát. Vui lòng thử lại.";

const TabPostPositionDraw = ({ rounds, onChange, readOnly }: Props) => {
  const allRaces = useMemo(
    () =>
      rounds.flatMap((round) => round.races.map((race) => ({ round, race }))),
    [rounds],
  );
  ...
  const raceId =
    selectedRaceId && /^\d+$/.test(selectedRaceId)
  ...
  const handleDraw = async () => {
    ...
    try {
      ...
      setSuccess(
        "Đã bốc thăm vị trí xuất phát. Danh sách đã được cập nhật từ hệ thống.",
      );
    } catch (requestError) {
      setError(friendlyError(requestError));
    } finally {
    ...
    return (
      <div className={styles.container}>
        <div className={styles.emptyState}>
          Chưa có cuộc đua nào. Vui lòng thêm vòng đấu và cuộc đua trước.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.raceSelectWrap}>
        <label className={styles.raceSelectLabel} htmlFor="post-position-race">
          Chọn cuộc đua
        </label>
        <select
          id="post-position-race"
        ...
        >
          {allRaces.map(({ round, race }) => (
            <option key={race.id} value={race.id}>
              {round.name} — Cuộc đua #{race.raceNumber}
      ...
      {!raceId && (
        <div className={styles.emptyState}>
          Hãy lưu giải đấu và cuộc đua trước khi thực hiện bốc thăm.
        </div>
      )}
      {error && (
      ...
      {selected && raceId && (
        <>
          <div className={styles.statusRow}>
            ...
            {!readOnly && (
              <button
                ...
                disabled={
                  drawing || loading || isDrawn || eligibleEntries.length < 2
                }
              >
                <FiShuffle size={15} />{" "}
                {drawing ? "Đang bốc thăm..." : "Bốc thăm vị trí"}
              </button>
            )}
          </div>
          ...
          {loading ? (
            <div className={styles.emptyState}>
              Đang tải danh sách xuất phát...
            </div>
          ) : entries.length === 0 ? (
            <div className={styles.emptyState}>
              Chưa có ngựa hợp lệ trong cuộc đua này để bốc thăm.
            </div>
          ) : (
            <div className={styles.entriesTable}>
              <div className={styles.entriesHeader}>
                <span>Tên ngựa</span>
                <span>Nài ngựa</span>
                <span>Vị trí xuất phát</span>
              </div>
              {entries.map((entry) => (
                <div key={entry.raceEntryId} className={styles.entryRow}>
                  <span className={styles.horseName}>{entry.horseName}</span>
                  <span>{entry.jockeyName}</span>
                  <span
                    className={`${styles.postPosition} ${entry.postPosition ? styles.postPositionAssigned : ""}`}
                  >
                    {entry.postPosition ?? "—"}
                  </span>
                </div>
