...
function toPayload(form: FormState): FamilyDeclarationPayload {
  const trimmedUserId = form.relatedUserId.trim();
  return {
    relatedPersonName: form.relatedPersonName.trim(),
    relatedUserId: trimmedUserId ? Number(trimmedUserId) : null,
...
export default function DoctorCoiDeclarations() {
  ...
  const loadDeclarations = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const data = await getFamilyDeclarations();
      setItems(data);
    } catch (err) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.relationType) {
      setFormError('Vui lòng chọn quan hệ.');
      return false;
    }
    if (!form.industryRole) {
  ...
  const handleEdit = (item: FamilyDeclaration) => {
    setEditing(item);
    setForm(toForm(item));
    setFormError('');
  };
  ...
  const handleDelete = async (item: FamilyDeclaration) => {
    ...
    const confirmed = window.confirm(`Xóa khai báo COI của ${relatedName}?`);
    if (!confirmed) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 border-b border-gray-200 pb-4">
        <h1 className="text-2xl font-bold text-gray-900">Khai báo quan hệ COI</h1>
        <p className="text-sm text-gray-500">
          Khai báo người thân hoặc quan hệ có thể ảnh hưởng đến phân công y tế trong giải đấu.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm xl:col-span-1">
          <div className="mb-4">
            ...
            <p className="mt-1 text-xs text-gray-500">
              Nhập người liên quan để hệ thống đối chiếu khi phân công race.
            </p>
          </div>
        ...
        <section className="rounded-lg border border-gray-200 bg-white shadow-sm xl:col-span-2">
          ...
          {error && (
            <div className="m-5 rounded-lg border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700">
              {error}
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Thêm người liên quan nếu có quan hệ gia đình hoặc lợi ích cần khai báo.
              </p>
            </div>
          ) : (
                    ...
                    return (
                      <tr key={item.declarationId} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-5 py-4 text-xs text-gray-500">
                          {formatDateTime(item.declaredAt)}
                        </td>
                        <td className="px-5 py-4">
