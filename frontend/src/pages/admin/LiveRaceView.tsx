...
const LiveRaceView = () => {
  ...
  const races = useMemo<RaceOption[]>(
    () =>
  ...
  useEffect(() => {
    getTournaments()
      .then((items) => {
        ...
        const allRaces = items.flatMap((tournament) =>
          tournament.rounds.flatMap((round) => round.races),
        );
        const liveRace = allRaces.find((race) => race.status === "Live");
        setSelectedRaceId(liveRace?.raceId ?? allRaces[0]?.raceId ?? null);
      })
      .catch(() =>
        setError("Không tải được danh sách cuộc đua. Vui lòng thử lại."),
      )
  ...
  const loadRace = async (raceId: number, background = false) => {
    ...
    setError("");
    try {
      const [raceStatus, raceViolations] = await Promise.all([
        getLiveRaceStatus(raceId),
      ...
      setError(
        requestError instanceof Error
          ...
          : "Không tải được dữ liệu cuộc đua.",
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => void loadRace(selectedRaceId, true),
      5_000,
    );
  ...
  return (
    <div className={styles.container}>
      <div className={styles.pageHeader}>
        <h1 className={styles.heading}>Theo dõi cuộc đua</h1>
      </div>
      <div className={styles.controls}>
        ...
        <button
          ...
          onClick={() => selectedRaceId && void loadRace(selectedRaceId, true)}
          disabled={!selectedRaceId || refreshing}
        >
          <FiRefreshCw size={15} />{" "}
      ...
      {loading ? (
        <div className={styles.card}>
          <p className={styles.loading}>Đang tải dữ liệu cuộc đua...</p>
        </div>
      ) : !selectedRaceId ? (
        <div className={styles.card}>
          <div className={styles.empty}>
            ...
            <h3 className={styles.emptyTitle}>Chưa có cuộc đua</h3>
            <p className={styles.emptyDesc}>
              Chưa có cuộc đua nào để theo dõi trực tiếp.
            </p>
          </div>
        </div>
      ) : (
          ...
          <>
            <section className={styles.summaryGrid}>
              <div className={styles.summaryItem}>
                <span>Cuộc đua</span>
                <strong>
                  {selectedRace?.label ?? `Cuộc đua #${status.raceId}`}
                </strong>
              </div>
              <div className={styles.summaryItem}>
              ...
              <div className={styles.summaryItem}>
                ...
                <strong>
                  {formatDateTime(
                    status.scheduledTime ?? selectedRace?.scheduledTime ?? null,
                  )}
                </strong>
              </div>
              <div className={styles.summaryItem}>
                <span>Bắt đầu thực tế</span>
                <strong>{formatDateTime(status.actualStartTime)}</strong>
              </div>
            </section>
            <p className={styles.updated}>
              ...
              {updatedAt
                ...
                : "Chưa có dữ liệu"}
              {status.status === "Live" ? " — tự động làm mới mỗi 5 giây." : ""}
            </p>

            <section className={styles.card}>
              <h2 className={styles.sectionTitle}>
                Danh sách ngựa và nài ngựa
              </h2>
              {status.entries.length === 0 ? (
                <p className={styles.emptyText}>
                  Chưa có dữ liệu danh sách xuất phát từ hệ thống.
                </p>
              ) : (
                        ...
                        <tr key={entry.raceEntryId}>
                          ...
                          <td>
                            {entry.finishPosition
                              ? `Hạng ${entry.finishPosition}${entry.finishTime ? ` — ${entry.finishTime}s` : ""}`
                              : "Chưa có dữ liệu"}
                          </td>
                        </tr>
                      ))}
            ...
            <section className={styles.card}>
              <h2 className={styles.sectionTitle}>Vi phạm đã ghi nhận</h2>
              {violations.length === 0 ? (
                <p className={styles.emptyText}>
                ...
                <ul className={styles.violations}>
                  {violations.map((violation) => (
                    <li key={violation.violationId}>
                      <strong>{violation.horseName}</strong>
                      <span>
                        {penaltyLabel(violation.penalty)} —{" "}
                        {violation.description || "Chưa có mô tả chi tiết."}
                      </span>
                      <small>{formatDateTime(violation.loggedAt)}</small>
                    </li>
                  ))}
                </ul>
              )}
