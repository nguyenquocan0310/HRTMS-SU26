  ...
  const renderEmpty = (title: string) => (
    <div className={styles.empty}>
      ...
      <p className={styles.emptyDesc}>
        Chưa có dữ liệu xếp hạng chính thức cho giải đấu và tiêu chí đã chọn.
      </p>
    </div>
  );
    ...
    <div className={styles.container}>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.subtext}>
          Theo dõi thành tích ngựa và nài ngựa từ dữ liệu chính thức.
        </p>
      </div>
          ...
          <section className={styles.tableCard}>
            <div className={styles.sectionHeader}>
              ...
              <p className={styles.sectionDesc}>
                Thành tích theo {mode === "points" ? "điểm" : "tiền thưởng"}.
              </p>
            </div>
            {loading ? (
              <p className={styles.loading}>Đang tải bảng xếp hạng ngựa...</p>
            ) : horses.length === 0 ? (
              renderEmpty("Chưa có xếp hạng ngựa")
            ) : (
                  ...
                  {horses.map((entry) => (
                    <tr key={entry.horseId}>
                      ...
                      <td className={styles.name}>
                        {entry.horseName ?? "Chưa có tên"}
                      </td>
                      <td>{entry.races}</td>
                      <td>{entry.wins}</td>
                      <td className={styles.points}>{entry.totalPoints}</td>
                      <td>{formatMoney(entry.totalEarnings)}</td>
                      <td>{entry.winRate.toLocaleString("vi-VN")}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className={styles.tableCard}>
            ...
            {loading ? (
              <p className={styles.loading}>
                Đang tải bảng xếp hạng nài ngựa...
              </p>
            ) : jockeys.length === 0 ? (
              renderEmpty("Chưa có xếp hạng nài ngựa")
            ) : (
              <table className={styles.table}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {jockeys.map((entry) => (
                    <tr key={entry.jockeyId}>
                      <td className={styles.rank}>#{entry.rank}</td>
                      <td className={styles.name}>
                        {entry.jockeyName ?? "Chưa có tên"}
                      </td>
                      <td>{entry.races}</td>
                      <td>{entry.wins}</td>
                      <td className={styles.points}>{entry.totalPoints}</td>
