...
const actionMeta = (
  ...
  if (a.includes("login"))
    return { icon: <FiUser size={16} />, title: "Đăng nhập" };
  if (a.includes("status"))
    return { icon: <FiShuffle size={16} />, title: "Đổi trạng thái" };
  if (a.includes("approve") || a.includes("duyệt"))
    return { icon: <FiCheckCircle size={16} />, title: "Duyệt hồ sơ" };
  if (a.includes("tournament"))
    return { icon: <FiFlag size={16} />, title: "Giải đấu" };
  if (a.includes("result") || a.includes("official"))
    return { icon: <FiAward size={16} />, title: "Kết quả" };
          ...
          <div className={styles.tourList}>
            {tournaments.map((t) => (
              <button
                ...
                onClick={() =>
                  navigate(`/admin/tournament-builder/${t.tournamentId}`)
                }
              >
                <div className={styles.tourInfo}>
                  <span className={styles.tourName}>{t.name}</span>
                  <span className={styles.tourMeta}>
                    {t.allowedBreed} · {formatDate(t.startDate)} –{" "}
                    {formatDate(t.endDate)}
                  </span>
                </div>
                <div className={styles.tourRight}>
                  <span className={styles.tourStatus}>
        ...
        <div className={styles.activityHeader}>
          <div>
            ...
            <p className={styles.activitySubtext}>
              Latest audit events from the backend audit log.
            </p>
          </div>
