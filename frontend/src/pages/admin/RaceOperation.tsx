...
const RaceOperations = () => {
      ...
      .then((data) => {
        ...
        if (
          ...
          (initialTournament.tournamentId !== selectedTournamentId ||
            initialRound.roundId !== selectedRoundId)
        ) {
  ...
  const allocate = async () => {
    ...
    try {
      ...
      setNotice(
        `Đã phân ${result.allocatedCount}/${result.poolSize} cặp vào cuộc đua. ${result.waitlistedCount} cặp đang chờ.`,
      );
      await refreshRound();
    } catch (err) {
  ...
  const finalize = async () => {
    ...
    try {
      ...
      setNotice(
        `Đã hoàn tất điều hành vòng đấu: phân ${result.allocation.allocatedCount} cặp và bốc thăm ${result.draws.length} cuộc đua.`,
      );
      await refreshRound();
    } catch (err) {
      setError(adminError(err));
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <div>
          ...
          <p>
            Chọn giải và vòng để theo dõi lệ phí, phân cuộc đua, bốc thăm và
            phân công cán bộ trong cùng một luồng.
          </p>
        </div>
        <button
      ...
      <section className={styles.card}>
        ...
        {tournament && (
          <div className={styles.venue}>
            <span className={styles.venueName}>
              <FiMapPin /> {tournament.venueName ?? "Chưa gán trường đua"}
            ...
            <span>
              Sức chứa mỗi cuộc đua: <strong>{capacity ?? "—"}</strong>
            </span>
          </div>
        )}
        ...
        <div
          ...
          aria-label="Tiến độ điều hành vòng đấu"
        >
          <span className={styles.done}>
            <FiCheck /> 1. Lệ phí đã sẵn sàng
          </span>
          <span className={hasAllocation ? styles.done : styles.current}>
            <FiCheck /> 2. Phân vào cuộc đua{" "}
            {hasAllocation ? "đã chốt" : "chưa chốt"}
          </span>
          <span className={allDrawn ? styles.done : styles.pending}>
            3. Bốc thăm {allDrawn ? "đã hoàn tất" : "chưa thực hiện"}
          </span>
          <span
            className={
              schedules.some((race) => race.status === "Official")
                ? styles.done
                : styles.pending
            }
          >
            4. Trọng tài kiểm tra từng cuộc đua
          </span>
        </div>
        ...
        <div className={styles.actions}>
          <button
            ...
            disabled={working || !canAllocate}
            onClick={() => setAllocateConfirm(true)}
          >
            <FiFlag /> Phân race
          </button>
          <button
      ...
      {loading || loadingRound ? (
        <section className={styles.card}>
          <p className={styles.empty}>Đang tải dữ liệu vòng đấu…</p>
        </section>
      ) : !round ? (
        <section className={styles.card}>
          <p className={styles.empty}>
            Chọn giải đấu và vòng đấu để bắt đầu điều hành.
          </p>
        </section>
      ) : (
        <>
          <section className={styles.card}>
            ...
            {hasAllocation ? (
              <p className={styles.previewText}>
                Danh sách đã được chốt. Bạn có thể điều chỉnh ngựa giữa các cuộc
                đua trước khi bốc thăm.
              </p>
            ) : (
              <p className={styles.empty}>
                Chưa có cặp đấu nào đủ điều kiện (đã xác nhận lệ phí) để phân
                vào cuộc đua.
              </p>
            )}
          </section>
          ...
          <section className={styles.card}>
            <div className={styles.sectionTitle}>
              <div>
                <h2>
                  Danh sách chờ <span>{waitlist.length}</span>
                </h2>
                <p>
                  Cặp đấu đủ điều kiện nhưng vượt sức chứa của vòng. Thứ tự là
                  thứ tự gọi bù.
                </p>
              </div>
            </div>
                ...
                <header>
                  <div>
                    <h2>
                      ...
                      {dateTime(race.scheduledTime)}
                    </h2>
                    <p>
                      {race.isPostPositionDrawn
                        ? "Đã bốc thăm vị trí xuất phát."
                        : "Chưa bốc thăm — vị trí xuất phát để trống."}
                    </p>
                  </div>
                  <span>
                    {race.entries.length}/{race.raceCapacity ?? capacity ?? "—"}
                  </span>
                </header>

                {race.entries.length === 0 ? (
                  <p className={styles.empty}>
                    Chưa có ngựa nào trong cuộc đua này.
                  </p>
                ) : (
                  <ul>
                    {race.entries
                      ...
                      .sort(
                        (a, b) =>
                          (a.postPosition ?? Number.MAX_SAFE_INTEGER) -
                          (b.postPosition ?? Number.MAX_SAFE_INTEGER),
                      )
                      .map((entry) => (
      ...
      {allocateConfirm && (
        <Modal
          ...
          loading={working}
        >
          Hệ thống sẽ chốt danh sách cặp đã xác nhận lệ phí, tự động phân toàn
          bộ vào các cuộc đua của vòng và lập danh sách chờ nếu vượt sức chứa.
        </Modal>
      )}
      ...
      {finalizeConfirm && (
        <Modal
          ...
          loading={working}
        >
          Hệ thống sẽ phân các cặp đủ điều kiện và bốc thăm độc lập từng cuộc
          đua. Cuộc đua không đủ điều kiện sẽ được giữ nguyên để xử lý sau.
        </Modal>
      )}
