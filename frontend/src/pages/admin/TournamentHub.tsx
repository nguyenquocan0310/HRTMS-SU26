...
const statusLabel = (s: string) => {
  const map: Record<string, string> = {
    ...
    Completed: "Completed",
    Cancelled: "Cancelled",
    "Closed Registration": "Closed registration",
    "Pre-Race": "Pre-race",
...
const TournamentHub = () => {
  ...
  useEffect(() => {
    getTournaments()
      .then(async (list) => {
        ...
        await Promise.allSettled(
          allRaces.map(async (race) => {
            try {
              results[race.raceId] = await getRaceSchedule(race.raceId);
            } catch {
              /* A schedule is optional in this overview. */
            }
          }),
        );
      ...
      .catch(() => {
        /* The empty-state handles an unavailable tournament list. */
      })
    ...
    <div className={styles.container}>
      ...
      <div className={styles.listCard}>
        ...
        {tournaments.length === 0 && (
          <p className={styles.empty}>Chưa có giải đấu nào.</p>
        )}
        ...
        {tournaments.map((t) => (
          <div key={t.tournamentId} className={styles.tournamentBlock}>
            ...
            <div className={styles.tournamentHeader}>
              <div>
                ...
                <span className={styles.tournamentMeta}>
                  {t.allowedBreed} | {formatDateTime(t.startDate)}
                </span>
              </div>
            ...
            {(t.status === "OpenRegistration" ||
              t.status === "Open Registration") && (
                ...
                <div className={styles.regLeft}>
                  ...
                  <div>
                    ...
                    <p className={styles.regDesc}>
                      Giải đang mở đăng ký. Quản trị viên có thể theo dõi và
                      điều chỉnh cấu hình giải.
                    </p>
                  </div>
                </div>
                <Link
                  className={styles.registerBtn}
                  to={`/admin/tournament-builder/${t.tournamentId}`}
                >
                  Mở trang quản lý
                </Link>
              </div>
            ...
            <div className={styles.statsGrid}>
              <div className={styles.statCard}>
                <span className={styles.statLabel}>Breed</span>
                <strong>{t.allowedBreed}</strong>
              </div>
              <div className={styles.statCard}>
                <span className={styles.statLabel}>Max horses</span>
                <strong>{t.maxHorses}</strong>
              </div>
              <div className={styles.statCard}>
                <span className={styles.statLabel}>Entry fee</span>
                <strong>{formatVND(t.entryFeeAmount)}</strong>
              </div>
              <div className={styles.statCard}>
                <span className={styles.statLabel}>Purse</span>
                <strong>{formatVND(t.purseAmount)}</strong>
              </div>
            </div>
                      ...
                      <div className={styles.raceHeader}>
                        <div>
                          ...
                          <p className={styles.raceMeta}>
                            {formatDateTime(race.scheduledTime)} |{" "}
                            {race.raceDistanceOverride ?? t.raceDistance}m
                          </p>
                        </div>
                        <span className={styles.upcomingBadge}>
