...
const RaceList = () => {
  ...
  const loadRaces = async (id: number) => {
    ...
    try {
      ...
      const schedules = await Promise.all(
        list.map(async (race) => {
          try {
            return await getRaceSchedule(race.raceId);
          } catch {
            return null;
          }
        }),
      ...
      const rows = list.map((race, index) => ({
        ...race,
      ...
      setSelectedRaceId((current) =>
        rows.some((race) => race.raceId === current)
          ? current
          : (rows[0]?.raceId ?? null),
      );
    } catch (err) {
  ...
  const declareOfficial = async (race: RaceRow) => {
    if (
      !window.confirm(
        `Chuyển cuộc đua #${race.raceNumber} sang trạng thái chính thức?`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setNotice(
        `Cuộc đua #${race.raceNumber} đã chuyển sang trạng thái chính thức.`,
      );
    ...
    <div className={styles.page}>
      ...
      <section className={styles.card}>
        <div className={styles.filters}>
          ...
          <label>
            ...
            <div>
              ...
              >
                <option value="">
                  -- Không có cuộc đua chưa chính thức --
                </option>

                {races.map((race) => (
                  <option key={race.raceId} value={race.raceId}>
                    {race.roundName} · Cuộc đua #{race.raceNumber}
                  </option>
                ))}
              </select>
        ...
        {selected && (
          <div className={styles.selected}>
            <strong>
              {selected.roundName} · Cuộc đua #{selected.raceNumber}
            </strong>

            <span>{dateTime(selected.scheduledTime)}</span>
            <span>{selected.entryCount} ngựa đăng ký</span>
            ...
            <button onClick={() => setDetail(selected)}>
              <FiEye /> Xem chi tiết
            </button>
          </div>
        )}
      ...
      <section className={styles.card}>
        <div className={styles.titleRow}>
          <h2>Cuộc đua chưa chính thức</h2>
            ...
            <table>
              ...
              <tbody>
                {races.map((race) => (
                  <tr key={race.raceId}>
                    <td>
                      <strong>Cuộc đua #{race.raceNumber}</strong>
                    </td>
      ...
      {detail && (
        <div className={styles.modalLayer}>
          <div className={styles.modal}>
            <h2>Cuộc đua #{detail.raceNumber}</h2>

            <dl>
              <dt>Vòng đấu</dt>
              <dd>{detail.roundName}</dd>

              <dt>Thời gian</dt>
              <dd>{dateTime(detail.scheduledTime)}</dd>

              <dt>Số ngựa</dt>
              <dd>{detail.entryCount}</dd>

              <dt>Trạng thái</dt>
              <dd>Chưa chính thức</dd>
            </dl>
            ...
            <div>
              ...
              <button
                ...
                disabled={!detail.canDeclareOfficial || declaring}
