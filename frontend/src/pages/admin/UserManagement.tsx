...
type UserRole =
  "Admin" | "Owner" | "Jockey" | "Referee" | "Doctor" | "Spectator";
...
const PAGE_SIZE = 8;
const isIdentityRole = (role: UserRole) =>
  ["Owner", "Jockey", "Referee", "Doctor"].includes(role);
const friendlyError = (error: unknown) => {
  const message = error instanceof Error ? error.message : "";
  if (/forbidden|403/i.test(message))
    return "Bạn không có quyền thực hiện thao tác này.";
  if (/not found|404/i.test(message))
    return "Không tìm thấy tài khoản. Danh sách đã được làm mới.";
  if (/already active|already suspended|409/i.test(message))
    return "Trạng thái tài khoản đã thay đổi. Vui lòng tải lại danh sách.";
  return message || "Không thể thực hiện thao tác. Vui lòng thử lại.";
};
const APPROVAL_ENDPOINTS: Partial<Record<UserRole, string>> = {
  Jockey: "jockeys",
  Referee: "referees",
  Doctor: "doctors",
};
...
const UserManagement = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiFetch<{ success: boolean; data: ApiUser[] }>(
        "/admin/users",
      );
      if (!response.success)
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (action === "activate") {
        ...
        await apiFetch(`/admin/users/${user.id}/activate`, { method: "PATCH" });
      } else if (action === "suspend") {
        // Tạm ngưng tài khoản người dùng.
        await apiFetch(`/admin/users/${user.id}/suspend`, { method: "PATCH" });
      } else {
        const endpoint = APPROVAL_ENDPOINTS[user.role];
        if (!endpoint)
          throw new Error("Không hỗ trợ phê duyệt cho vai trò này.");
        // Duyệt hoặc từ chối hồ sơ.
        await apiFetch(`/admin/${endpoint}/${user.id}/${action}`, {
          method: "PATCH",
        });
      }
      setMessage(
  ...
  const handleCreate = async (event: React.FormEvent) => {
    ...
    if (
      ...
      setError(
        "Vui lòng nhập tên đăng nhập, họ tên, email hợp lệ và mật khẩu từ 6 ký tự.",
      );
      return;
    }
    if (
      isIdentityRole(form.role) &&
      (!form.phoneNumber.trim() ||
        !form.dateOfBirth ||
        !/^\d{12}$/.test(form.identityNumber))
    ) {
      setError(
        "Vai trò đã chọn yêu cầu số điện thoại, ngày sinh và CCCD gồm 12 chữ số.",
      );
      return;
    }
    setCreating(true);
    try {
      ...
      await apiFetch("/admin/users", {
        ...
        body: JSON.stringify({
          ...
          experienceYears: form.experienceYears
            ...
            : undefined,
          selfDeclaredWeight: form.selfDeclaredWeight
      ...
      setShowCreate(false);
      setForm(EMPTY_FORM);
      setMessage("Đã tạo tài khoản mới. Mật khẩu không được hiển thị lại.");
      await fetchUsers();
    } catch (requestError) {
      setError(friendlyError(requestError));
    } finally {
  ...
  const columns: DataTableColumn<SystemUser>[] = [
    {
      ...
      render: (row) => (
        <span className={styles.usernameCell}>{row.username}</span>
      ),
    },
    {
      key: "email",
    ...
    },
    {
      ...
      header: "Vai trò",
      render: (row) => (
        <span className={styles.roleBadge}>{ROLE_LABELS[row.role]}</span>
      ),
    },
    {
      key: "status",
    ...
    },
    {
      ...
      render: (row) => (
        <div className={styles.actionCell}>
          {row.status === "Pending" &&
            ["Jockey", "Referee", "Doctor"].includes(row.role) && (
              <button
                type="button"
                className={styles.approveBtn}
                disabled={actionBusy}
                onClick={() =>
                  setPendingAction({ user: row, action: "approve" })
                }
              >
                Phê duyệt
              </button>
          ...
          {row.status === "Suspended" && (
            <button
              ...
              disabled={actionBusy}
              onClick={() =>
                setPendingAction({ user: row, action: "activate" })
              }
            >
              Kích hoạt
            </button>
    ...
    <div className={styles.container}>
        ...
        <div className={styles.filters}>
          <div className={styles.searchWrap}>
            ...
            <input
              ...
              value={searchQuery}
              onChange={(event) =>
                setFilter(() => setSearchQuery(event.target.value))
              }
            />
          </div>
          <select
      ...
      {pendingAction && (
        <ConfirmDialog
          ...
          message={`Bạn có chắc muốn thực hiện thao tác này với tài khoản "${pendingAction.user.username}"?`}
          confirmLabel={actionBusy ? "Đang xử lý..." : "Xác nhận"}
          variant={
            pendingAction.action === "suspend" ||
      ...
      {detailUser && (
        <UserDetailModal
          ...
          basicInfo={{
            ...
            role: ROLE_LABELS[detailUser.role],
            status: detailUser.status,
            joinedDate: detailUser.joinedDate,
          }}
          showActions={
            detailUser.status === "Pending" &&
            ["Jockey", "Referee", "Doctor"].includes(detailUser.role)
          }
          onApprove={() =>
            setPendingAction({ user: detailUser, action: "approve" })
          }
          onReject={() =>
            setPendingAction({ user: detailUser, action: "reject" })
          }
      ...
      {showCreate && (
        ...
        >
          ...
          >
            <h2>Tạo tài khoản</h2>
            <p>Chỉ các trường được backend hỗ trợ được gửi đi.</p>
            <div className={styles.formGrid}>
              ...
              {form.role === "Jockey" && (
                <>
                  ...
                  <label>
                    Cân nặng khai báo (kg)
                    <input
                      type="number"
                      min="0"
                      step="0.1"
                      value={form.selfDeclaredWeight}
                      onChange={(event) =>
                        setForm({
                          ...form,
                          selfDeclaredWeight: event.target.value,
                        })
                      }
                    />
                  </label>
