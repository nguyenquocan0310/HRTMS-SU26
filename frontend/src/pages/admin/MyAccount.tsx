...
const MyAccount = () => {
  ...
  const loadProfile = async () => {
    setLoading(true);
    try {
      const data = await getMyAccountProfile<Record<string, unknown>>();
      setProfile(data);
  ...
  const handleProfileSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setProfile((previous) =>
        previous
          ? { ...previous, fullName: fullName.trim(), email: email.trim() }
          : previous,
      );
  ...
  const handlePasswordSubmit = async (event: React.FormEvent) => {
    ...
    if (!currentPassword || !newPassword || !confirmPassword)
      return setError("Vui lòng nhập đầy đủ thông tin đổi mật khẩu.");
    if (newPassword.length < 6)
      return setError("Mật khẩu mới phải có ít nhất 6 ký tự.");
    if (newPassword !== confirmPassword)
      return setError("Xác nhận mật khẩu mới chưa khớp.");
    ...
    try {
      ...
      setConfirmPassword("");
      setMessage(
        "Đã đổi mật khẩu thành công. Phiên đăng nhập hiện tại vẫn được giữ theo cơ chế của hệ thống.",
      );
    } catch (requestError) {
      setError(
        requestError instanceof Error
  ...
  return (
    <div className={styles.container}>
      ...
      {loading ? (
        <p className={styles.loading}>Đang tải thông tin tài khoản...</p>
      ) : (
        profile && (
          <>
