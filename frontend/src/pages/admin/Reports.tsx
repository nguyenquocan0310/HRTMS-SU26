...
const REPORT_OPTIONS: Array<{
...
}> = [
  {
    ...
    label: "Kết quả giải đấu",
    description: "Tổng hợp kết quả chính thức trong giải.",
  },
  {
    value: "race-results",
    label: "Kết quả cuộc đua",
    description: "Chi tiết kết quả của từng cuộc đua.",
  },
  {
    value: "purse-payouts",
    label: "Chi trả tiền thưởng",
    description: "Các khoản thưởng đã được hệ thống ghi nhận.",
  },
  {
    value: "entry-list",
    label: "Danh sách đăng ký",
    description: "Danh sách ngựa và nài ngựa tham gia.",
  },
];
...
const Reports = () => {
  ...
  const loadPreview = async () => {
    ...
    setError("");
    try {
      setPreview(await getReportPreview(type, tournamentId));
    } catch (requestError) {
      setPreview(null);
      setError(
  ...
  useEffect(() => {
    if (tournamentId) void loadPreview();
  }, [type, tournamentId]);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          <h1>Báo cáo</h1>
          <p>Xem trước và xuất dữ liệu CSV do hệ thống cung cấp.</p>
        </div>
        <button
          type="button"
          className={styles.exportBtn}
          onClick={() => void handleExport()}
          disabled={!tournamentId || exporting}
        >
          <FiDownload size={15} /> {exporting ? "Đang xuất..." : "Xuất CSV"}
        </button>
      </div>
      ...
      <section className={styles.card}>
        <h2>Xem trước</h2>
        {loading ? (
          <p className={styles.empty}>Đang tải báo cáo...</p>
        ) : !preview ? (
          <p className={styles.empty}>Chưa có dữ liệu báo cáo.</p>
        ) : preview.rows.length === 0 ? (
          <p className={styles.empty}>
            Báo cáo chưa có dữ liệu phù hợp với giải đấu đã chọn.
          </p>
        ) : (
                ...
                {preview.rows.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    {preview.headers.map((header, index) => (
                      <td key={`${rowIndex}-${header}`}>{row[index] ?? "—"}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
