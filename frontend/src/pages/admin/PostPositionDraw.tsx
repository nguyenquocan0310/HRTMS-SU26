...
const PostPositionDraw = () => {
  ...
  const select = (nextTournament?: number, nextRound?: number) => {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setRaces(result.sort((a, b) => a.raceNumber - b.raceNumber));
    } catch (err) {
  ...
  const draw = async () => {
    ...
    setDrawingId(confirmRace.raceId);
    setError("");
    try {
      await drawPostPositions(confirmRace.raceId);
      setNotice(
        `Đã bốc thăm vị trí xuất phát cho cuộc đua #${confirmRace.raceNumber}.`,
      );
      setConfirmRace(null);
      await refresh();
    } catch (err) {
                ...
                {race.entries.length === 0 ? (
                  ...
                  <ul>
                    {race.entries
                      ...
                      .sort(
                        (a, b) =>
                          (a.postPosition ?? 999) - (b.postPosition ?? 999),
                      )
                      .map((entry) => (
