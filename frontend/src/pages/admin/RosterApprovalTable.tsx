...
const RosterApprovalTable = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<ParticipantResponse | null>(
    null,
  );
  ...
  }, [selectedTournamentId, roleFilter, statusFilter]);
  ...
  const handleApprove = async () => {
    ...
    try {
      const updated = await approveParticipant(selectedItem.participantId);

      setItems((currentItems) =>
        currentItems.map((item) =>
          item.participantId === selectedItem.participantId ? updated : item,
        ),
      );
  ...
  const columns: DataTableColumn<ParticipantResponse>[] = [
    {
      ...
      render: (row) => <span className={styles.name}>{row.fullName}</span>,
    },
    {
      key: "email",
      header: "Email",
  ...
  return (
    <div className={styles.wrap}>
      ...
      {loading ? (
        <p className={styles.loading}>Đang tải danh sách đăng ký...</p>
      ) : (
        <DataTable
          columns={columns}
          data={items}
      ...
      {selectedItem && (
        <UserDetailModal
          userId={String(selectedItem.userId)}
          role={selectedItem.role}
          basicInfo={{
            username: selectedItem.email.split("@")[0] || "—",
            "Full Name": selectedItem.fullName,
            email: selectedItem.email,
            role: selectedItem.role,
            status: selectedItem.status,
            joinedDate: formatDate(selectedItem.registeredAt),
            Tournament: selectedItem.tournamentName ?? "—",
          }}
          showActions={selectedItem.status === "Pending" && !actionLoading}
