...
const StepReview = ({ formData, onGoToStep }: Props) => {
  ...
  // Bước 1
  items.push({ label: 'Role', value: formData.role ?? '—', step: 1 });

  // Bước 2
  items.push({ label: 'Username', value: formData.identity.username || '—', step: 2 });
  items.push({ label: 'Email', value: formData.identity.email || '—', step: 2 });

  // Bước 3
  items.push({ label: 'Password', value: '••••••••', step: 3 });

  // Bước 4 — theo role
  if (formData.role === RegRole.Owner) {
    items.push({ label: 'Phone Number', value: formData.ownerVerification.phoneNumber || '—', step: 4 });
    items.push({ label: 'Identity Number', value: formData.ownerVerification.identityNumber || '—', step: 4 });
  }
